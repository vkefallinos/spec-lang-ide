...
import { DiagnosticRelatedInformation } from "./types";
...
//   ) {
//     super(worker);
//     this._onNewDiagnostics = onNewDiagnostics;
...
//       return;
//     }
//     const markers = diagnostics.map((d) => this._convertDiagnostics(model, d));

//     editor.setModelMarkers(model, this._selector, markers);
//     this?._onNewDiagnostics(markers, model);
//   }
...
export class DiagnosticsAdapter extends Adapter {
  ...
  constructor(
      ...
      const visibleSubscription = model.onDidChangeAttached(() => {
        ...
        if (onlyVisible) {
          if (model.isAttachedToEditor()) {
            ...
            // this model is no longer attached to an editor
            // => clear existing diagnostics
            this._editor.setModelMarkers(model, this._selector, []);
          }
        }
      });
    ...
    this._disposables.push(
      this._editor.onDidCreateModel((model) =>
        onModelAdd(<IInternalEditorModel>model)
      )
    );
    this._disposables.push(this._editor.onWillDisposeModel(onModelRemoved));
    this._disposables.push(
      this._editor.onDidChangeModelLanguage((event) => {
        onModelRemoved(event.model);
        onModelAdd(<IInternalEditorModel>event.model);
      })
    );
    ...
    this._disposables.push({
      dispose() {
        for (const model of this._editor.getModels()) {
          onModelRemoved(model);
        }
      }
    });
  ...
  private async _doValidate(model: editor.ITextModel): Promise<void> {
    ...
    const monacoMarkers = diagnostics.map((d) =>
      this._convertDiagnostics(model, d)
    );
    this?.onNewDiagnostics(monacoMarkers, model);
  }
